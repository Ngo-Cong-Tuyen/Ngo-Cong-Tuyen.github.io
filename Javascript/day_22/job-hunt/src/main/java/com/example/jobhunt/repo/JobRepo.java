...
public class JobRepo  {
    ...
    // Trong constructor của class, gọi method initJob() để khởi tạo các đối tượng và thêm vào list
    // Inject CompanyRepo vào để sử dụng (inject qua constructor)
    ...
    public void delete(int id) {
        findById(id).orElseThrow(()->{
            throw new NotFoundException("Khong tim thay job co id= "+id);
        });
    ...
    public void updateJobById(int id, UpdateJobRequest request) {
        Job job = findById(id).orElseThrow(()->{
            throw new NotFoundException("Khong tim thay job co id= "+id);
        });
